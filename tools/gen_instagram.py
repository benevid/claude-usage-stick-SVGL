...
def cta_slide(n, total, tag):
    ...
    body += sub(850, ["Firmware aberto, gravação pelo navegador,", "case pra imprimir em 3D."], 32)
...
def set_projeto():
    ...
    b += sub(600, ["Uma tela touch de 3,5\" que mostra, em tempo real,", "quanto da sua cota já foi."], 32)
    ...
    b += sub(640, ["Quem usa Claude Code no plano Pro ou Max vive", "com essa pergunta. A resposta fica escondida", "num comando de terminal — e você só descobre", "que acabou quando trava."], 32)
    ...
    b += bullets(1110, [("Sonda real na API, 1 modelo por ciclo", "piscando quando está tudo bem, suando quando o limite aperta")], 30)
    out.append(frame(b, 4, T, tag=tag))
    # 5 janela
    b = kicker(150, "TELA 3 • JANELA DE 5H")
    b += headline(260, ["Projeção de", "esgotamento."], 88)
    b += device(asset("mock-janela5h.png"), 90, 420, 900)
    b += bullets(1110, [("Uso real + linha pontilhada até o 100%", "\"no ritmo atual, esgota às 16:12\" — antes de acontecer")], 30)
    out.append(frame(b, 5, T, tag=tag))
    # 6 ritmo
    b = kicker(150, "TELA 4 • RITMO")
    b += headline(260, ["Que horas você", "queima mais?"], 88)
    b += device(asset("mock-ritmo.png"), 90, 420, 900)
    b += bullets(1110, [("Heatmap por hora local", "hoje, 7 dias, 30 dias ou tudo — histórico fica no device")], 30)
    ...
    b += bullets(1010, [("Animação nos 25 / 50 / 70 / 100%", "e até 4 contas, cada uma com seu histórico"),
                        ("PT-BR e EN, PIN, token cifrado AES-256", "nada sai do device além da chamada à API")], 28, gap=104)
...
def set_logo():
    ...
    b += headline(260, ["Clawd de um lado,", "você no centro."], 88)
    ...
    b += bullets(860, [("Logo centralizada, 36 px de altura", "o mascote Claude continua à esquerda"),
                       ("Aparece também na tela Sobre", "o resto do gadget é exatamente o mesmo")], 30, gap=120)
    out.append(frame(b, 2, T, tag=tag))
    # 3 sem recompilar
    b = kicker(150, "SEM COMPLICAÇÃO")
    b += headline(260, ["Manda o PNG.", "Só isso."], 88)
    b += sub(470, ["A logo entra no firmware na hora de gravar —", "sem recompilar nada, sem mexer em código."], 32)
    ...
    b += bullets(600, [("Empresas que usam Claude Code", "cada dev com o medidor — e a sua marca — na mesa"),
                       ("Agências e consultorias", "brinde técnico que ninguém guarda na gaveta"),
    ...
    b += sub(1080, ["Janelas, projeção, mascotes, histórico, multi-conta:", "tudo lá. Só o header muda."], 30)
    out.append(frame(b, 5, T, tag=tag))
...
def set_como_ter():
    ...
    out = []
    b = kicker(150, "DO ZERO À MESA")
    b += headline(260, ["Como ter", "o seu em", "3 passos."], 96)
    b += sub(600, ["Uma placa, um navegador e uma impressora 3D", "(essa última é opcional)."], 32)
    b += device(asset("mock-agora.png"), 130, 690, 820, tilt=-3)
    out.append(frame(b, 1, T, tag=tag))
    # 2 placa
    b = kicker(150, "PASSO 1 • A PLACA")
    b += headline(260, ["Guition", "JC4832W535"], 88)
    b += sub(470, ["ESP32-S3 com tela IPS touch de 3,5\" (480x320).", "Custa pouco, vem pronta, só liga no USB."], 32)
    b += f'<rect x="60" y="620" width="{W - 120}" height="420" rx="24" fill="{SURF}"/>'
    specs = [("chip", "ESP32-S3, WiFi, 8 MB PSRAM"), ("tela", "3,5\" IPS 480x320 touch"),
             ("alimentação", "USB-C, 5V"), ("onde achar", "AliExpress, Mercado Livre")]
    ...
    b += headline(260, ["Seu token", "fica só ali."], 88)
    b += bullets(500, [("Token cifrado com AES-256-GCM", "chave derivada de um PIN de 4 dígitos que nunca é salvo"),
                       ("10 erros de PIN = apaga tudo", "e o tempo de espera dobra a cada erro"),
                       ("Fala só com api.anthropic.com", "sem backend, sem nuvem, sem conta")], 32, gap=150)
    out.append(frame(b, 4, T, tag=tag))
    # 5 case
    b = kicker(150, "PASSO 3 • O CASE")
    b += headline(260, ["Três cases,", "escolha o seu."], 88)
    ...
    b += mono(600, 1070, "STL no repositório", 22, fill=MUTED)
    out.append(frame(b, 5, T, tag=tag))
    out.append(cta_slide(6, T, tag))
    return out
