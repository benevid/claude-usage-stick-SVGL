 ...
 * Placa: ESP32-2432S028 ("Cheap Yellow Display"), ESP32 comum, ILI9341 SPI,
 * touch XPT2046 resistivo. Port do firmware original (Guition JC4832W535,
 * ESP32-S3) — 320x240 paisagem em vez de 480x320.
 *
 ...
 * Init de display/touch validado no bring-up (ver firmware/bringup_cyd/).
 */
#include <Arduino.h>
#include <SPI.h>
#include <TFT_eSPI.h>
...
static void disp_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map) {
  ...
  gfx.pushPixels((uint16_t *)px_map, w * h);
  gfx.endWrite();
...
static void apply_brightness() { ledcWrite(TFT_BL_PIN, BRI_LEVELS[g_briIdx]); }
...
static void ui_pin() {
  ...
  lv_obj_align(g_pinDots, LV_ALIGN_TOP_MID, 0, 32);
  pin_update_dots();
  ...
  lv_obj_align(g_pinMsg, LV_ALIGN_TOP_MID, 0, 66);

  lv_obj_t *bm = lv_buttonmatrix_create(scr);
  lv_buttonmatrix_set_map(bm, pin_map);
  lv_obj_set_size(bm, 264, 150);
  lv_obj_align(bm, LV_ALIGN_BOTTOM_MID, 0, -4);
...
static void ui_wifi() {
  lv_obj_t *scr = lv_screen_active();
  lv_obj_t *title = mklabel(scr, TRS("Configurar WiFi", "Configure WiFi"), &lv_font_montserrat_14, C_TEXT);
  ...
  if (!g_onboarding && g_hasToken) {
    ...
    lv_obj_align(bk, LV_ALIGN_TOP_RIGHT, -98, 4);
  ...
  lv_obj_align(wifi_status, LV_ALIGN_TOP_LEFT, 8, 30);

  wifi_list = lv_list_create(scr);
  lv_obj_set_size(wifi_list, 304, 154);
  ...
  lv_obj_set_size(wifi_ta, 304, 36);
  lv_obj_align(wifi_ta, LV_ALIGN_TOP_MID, 0, 48);
  lv_obj_add_flag(wifi_ta, LV_OBJ_FLAG_HIDDEN);

  wifi_kb = lv_keyboard_create(scr);
  lv_obj_set_size(wifi_kb, SCREEN_WIDTH, 118);
...
static void ui_token() {
  ...
  lv_obj_t *cap = mklabel(scr, TRS("Cole o token pelo navegador, em:",
                                   "Paste the token via browser, at:"), &lv_font_montserrat_14, C_MUTED);
  lv_obj_align(cap, LV_ALIGN_TOP_MID, 0, 96);

  String url = String("http://") + WiFi.localIP().toString();
  lv_obj_t *ip = mklabel(scr, url.c_str(), &lv_font_montserrat_20, C_ACCENT);
  lv_obj_align(ip, LV_ALIGN_TOP_MID, 0, 116);
  ...
  lv_obj_align(sp, LV_ALIGN_BOTTOM_MID, 0, -40);
  ...
  lv_obj_align(g_tokMsg, LV_ALIGN_BOTTOM_MID, 0, -10);
...
// Falha antes do LVGL existir: desenha direto na TFT_eSPI. Tambem acende o
...
static void build_win_card(lv_obj_t *t, int x, const char *title,
  ...
  *pct = tlabel(c, &lv_font_montserrat_28, C_OK, 0, 14);
  for (int i = 0; i < NSEG; i++)                    // medidor: 18 segmentos
    seg[i] = rrect(c, i * 6, 50, 4, 11, 1, C_TRACK);
  *at = tlabel(c, &lv_font_montserrat_12, C_FAINT, 0, 64);
  *cd = tlabel(c, &lv_font_montserrat_18, C_TEXT, 0, 80);
}
static void build_tile_agora(lv_obj_t *t) {
  ...
  g_ui.agChip = mkchip(t, 8, 138);
  g_ui.agTok = tlabel(t, &lv_font_montserrat_12, C_MUTED, 95, 144);
  lv_obj_set_width(g_ui.agTok, 217);
...
static void build_tile_models(lv_obj_t *t) {
  static const int CENTERS[NMODELS] = {40, 120, 200, 280};
  for (int i = 0; i < NMODELS; i++) {
    ...
    lv_obj_set_pos(n, CENTERS[i] - 38, 94);
    g_ui.mChip[i] = mkchip(t, 0, 112);
  }
  tstatic(t, TRS("sonda real na API \xE2\x80\xA2 1 modelo por ciclo",
                 "live API probe \xE2\x80\xA2 1 model per cycle"),
          &lv_font_montserrat_12, C_FAINT, 8, 138);
  g_ui.incident = tlabel(t, &lv_font_montserrat_12, C_MUTED, 8, 154);
  lv_obj_set_width(g_ui.incident, 304);
...
static void build_tile_trend(lv_obj_t *t) {
  tstatic(t, TRS("Janela de 5h", "5-hour window"), &lv_font_montserrat_16, C_TEXT, 8, 2);
  lv_obj_t *sub = tstatic(t, TRS("uso real + projecao", "real usage + projection"),
                          &lv_font_montserrat_12, C_FAINT, 150, 5);
  lv_obj_set_width(sub, 162);
  lv_obj_set_style_text_align(sub, LV_TEXT_ALIGN_RIGHT, 0);

  lv_obj_t *c = card(t, 10, 22, 300, 114);
  ...
  g_ui.trCap = tlabel(t, &lv_font_montserrat_12, C_MUTED, 10, 140);
...
// baseline/altura maxima das barras — compartilhado com heat_redraw()
#define HEAT_BASE_Y  130
#define HEAT_MAX_H   76
#define HEAT_BAR_W   8
#define HEAT_PITCH   12
#define HEAT_X0      10
static void build_tile_heat(lv_obj_t *t) {
  tstatic(t, TRS("Ritmo por hora", "Hourly rhythm"), &lv_font_montserrat_14, C_TEXT, 8, 8);
  for (int i = 0; i < 4; i++) {
    lv_obj_t *b = lv_button_create(t);
    lv_obj_set_size(b, 44, 26);
    lv_obj_set_pos(b, 118 + i * 50, 2);
  ...
  for (int h = 0; h < 24; h++) {
    ...
    lv_obj_set_pos(bar, HEAT_X0 + h * HEAT_PITCH, HEAT_BASE_Y);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    lv_obj_set_pos(l, HEAT_X0 + h * HEAT_PITCH - 2, HEAT_BASE_Y + 6);
  }
  tstatic(t, TRS("quota da janela 5h queimada em cada hora local",
                 "5h-window quota burned per local hour"), &lv_font_montserrat_12, C_FAINT, 8, 150);
}
...
static void heat_redraw() {
  ...
  for (int h = 0; h < 24; h++) {
    ...
    lv_obj_set_y(g_ui.heat[h], HEAT_BASE_Y - hgt);
...
static void ui_main() {
  ...
  // status/badge numa segunda linha (320px nao da pra tudo numa linha so),
  // botoes de atualizar/ajustes so-icone no canto superior direito.
  ...
  lv_obj_set_pos(hWord, 50, 4);

  lv_obj_t *logoSpot = lv_obj_create(scr);     // hotspot icone+nome (so demo)
  lv_obj_set_pos(logoSpot, 4, 2); lv_obj_set_size(logoSpot, 100, 30);
  ...
  // botao de atualizar (so-icone; a busca e bloqueante)
  lv_obj_t *ref = mkbtn(scr, LV_SYMBOL_REFRESH, &lv_font_montserrat_16, C_SURFACE2, C_ACCENT);
  lv_obj_set_size(ref, 34, 28);
  lv_obj_set_ext_click_area(ref, 8);
  lv_obj_align(ref, LV_ALIGN_TOP_RIGHT, -40, 4);
  ...
  if (accountCount(g_accts) > 1) {
    ...
    lv_obj_align(acct, LV_ALIGN_TOP_LEFT, 200, 34);
  }

  // Barra fina decrescente do próximo refresh (só indicador)
  g_ui.refBar = lv_bar_create(scr);
  lv_obj_set_size(g_ui.refBar, SCREEN_WIDTH, 3);
  lv_obj_set_pos(g_ui.refBar, 0, 50);
  ...
  lv_obj_set_pos(g_ui.tv, 0, 54);
  lv_obj_set_size(g_ui.tv, SCREEN_WIDTH, SCREEN_HEIGHT - 54 - 16);
...
void setup() {
  ...
  // LVGL — buffer parcial em RAM interna (esta placa nao tem PSRAM)
  lv_init();
  lv_tick_set_cb([]() -> uint32_t { return millis(); });
  static const uint32_t LV_BUF_LINES = 40;
  uint32_t bufSize = SCREEN_WIDTH * LV_BUF_LINES * sizeof(lv_color_t);
  lv_color_t *buf = (lv_color_t *)heap_caps_malloc(bufSize, MALLOC_CAP_DMA | MALLOC_CAP_8BIT);
  if (!buf) { Serial.println("FATAL RAM"); fatal_screen("RAM indisponivel"); }
